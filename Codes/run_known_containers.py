...
HOST_WORKLOAD_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../serverless-exp/workloads")
)
HOST_SEBS_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "serverless-benchmarks")
)
...
OUTPUT_FILE = os.environ.get(
    "SEBS_KNOWN_OUTPUT",
    os.environ.get(
        "MIBENCH_KNOWN_OUTPUT",
        "/home/said/Pictures/Paper_Conference/Paper_Micro/Codes/raw_sebs_known.jsonl",
    ),
)
WINDOWS_OUTPUT = os.environ.get(
    "SEBS_KNOWN_WINDOWS_OUTPUT",
    "/home/said/Pictures/Paper_Conference/Paper_Micro/Codes/raw_sebs_known_windows.jsonl",
)
...
WORKLOAD_SPECS = {
    "sebs_dynamic_html_known": {
        "script": "sebs_dynamic_html_known.py",
    ...
    "sebs_graph_bfs_known": {
        "script": "sebs_graph_bfs_known.py",
    ...
    "sebs_graph_mst_known": {
        "script": "sebs_graph_mst_known.py",
    ...
    "sebs_compression_known": {
        "script": "sebs_compression_known.py",
    ...
    "sebs_video_processing_known": {
        "script": "sebs_video_processing_known.py",
    ...
    "sebs_dna_visualisation_known": {
        "script": "sebs_dna_visualisation_known.py",
    ...
    "functionbench_download_upload_known": {
        "script": "functionbench_download_upload_known.py",
...
def main():
    ...
    selected_workloads = parse_list_env(
        ["SEBS_KNOWN_WORKLOADS", "MIBENCH_KNOWN_WORKLOADS"],
        KNOWN_WORKLOADS,
    )
    ...
    if invalid:
        print(f"Error: Unknown known workloads requested: {invalid}")
        return
    if not validate_required_images(selected_workloads):
        return

    if target_iterations is not None:
        print(f"--- COLLECTING SEBS KNOWN SERVERLESS DATA ({target_iterations} Iterations / Fixed-Work Mode) ---")
    else:
        print(f"--- COLLECTING SEBS KNOWN SERVERLESS DATA ({target_seconds}s Duration) ---")
        ...
        for i in range(iterations):
            ...
            for mem, cpu_limit in config_order:
                ...
                if host_budget_seconds > 0 and (elapsed_budget + expected_run_seconds) > host_budget_seconds:
                    print(
                        f"\n[STOP] Host budget reached for known collection "
